 ...
 * Analytics has no per-resource permissions, so only the role authorities
 * ({@code ROLE_*}) are granted; ingestion is gated to admins via method security.
 */
@Component
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private void authenticate(HttpServletRequest request, Claims claims) {
        ...
        for (String role : tokenProvider.roles(claims)) {
